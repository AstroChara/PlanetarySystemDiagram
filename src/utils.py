...
def find_P(a,unit_a,M,unit_M,unit_P):
...
def find_a(P,unit_P,M,unit_M,unit_a):
    P_SI = time_conversion(P,unit_P,'s')
    M_SI = mass_conversion(M,unit_M,'kg')
    
    a_SI = np.cbrt(G * M_SI * (P_SI / (2 * np.pi))**2 )
    a = length_conversion(a_SI,'m',unit_a)
    return a
